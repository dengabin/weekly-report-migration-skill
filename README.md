...
| Agent 自动完成 | 你只需要 |
|----------------|----------|
| 获取 / 安装 Skill 到本机 | 用 Cursor 打开对话 |
| 安装依赖、预检环境 | 首次提供两个云文档链接 |
...
| 3（少数情况） | 部门子表叫什么 | 从列表里选，如 `应用研发-AI应用组` |
...
| 现象 | 怎么办 |
|------|--------|
| 不知道从哪开始 | 对 Agent 说：`请安装周报迁移 Skill 并执行周报迁移` |
...
| [scripts/README.md](scripts/README.md) | 脚本说明（Agent 用） |
