...
1. 扫描表头中姓名列之后的所有日期列（`X月Y日`），按列位置排列后，将新周插入到第一个比自己旧的日期列位置（该列及右侧全部右移），保持日期降序（最新在左、最旧在右）
2. 表头单元格文案参考相邻列，例如：  
   `7月2日` + 换行 + `周期：6.26-7.2`（周期从 `config.week_aliases` 推断）
3. 若表头无任何日期列，回退到姓名列之后第一个位置插入
