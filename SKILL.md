...
5. 根据 `status` 分支：
   ...
   | `need_team_name` | `resolve_team_name.py` 退出码 3 → 读 `team-resolve.json`：`flat_sheet`+姓名 `not_found` → AskQuestion 是否 otl 笔误；`ambiguous` → AskQuestion 页内组名；**禁止猜选** |
...
7. 小组 otl → `extract_otl_weekly.py` → `build_config_from_extracted.py` → **`resolve_team_name.py`**（平铺表 `flat_sheet` 不问组名；多组分区唯一对应则不问；否则 AskQuestion，见 [team-name-resolution.md](references/team-name-resolution.md)）。
...
| 检查项 | 说明 |
|--------|------|
| Python 依赖 | `openpyxl`（xlsx 备选路径）、`pyyaml`（auth.yaml）；见 `requirements.txt` |
...
- ❌ 周次（用户未主动说时）→ 默认最新一期
- ❌ 组名 — 平铺表不问；多组分区仅当 `resolve_team_name.py` 无法唯一确定时才 AskQuestion（见 [team-name-resolution.md](references/team-name-resolution.md)）
...
| `scripts/plan/resolve_team_name.py` | 判平铺/多组 → 平铺按姓名校验或组名反推；退出码 3 时读 `team-resolve.json` |
| `scripts/lib/column_width.py` | 周列宽度估算（插入列与写回后加宽，带上限） |
...
| 子表对了但人找不到 | 平铺表：核对 otl `## 姓名` 与部门表是否一致；多组表：检查 `week`、 `team_row_marker`；见 [team-name-resolution.md](references/team-name-resolution.md) |
| 误报「多个组」让你选员工名 | 应为平铺表；确认 `dept_sheet.layout=flat` 或子表 tab 正确 |
